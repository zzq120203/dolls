	...
	public RedisPoolSelector(MMConf conf, RedisPool rpL1) throws Exception {
		...
		case SENTINEL:
			try {
				...
				// get cn.ac.iie.client alloc policy
	...
	private String __createSet(String set) throws RException {
		...
		try {
			...
			if (id != null) {
				...
				//create set rocksid
				String rocksnum = jedis.hget("mm.cn.ac.iie.client.conf", "rocksnum");
                Random random = new Random();
                int rocksid = random.nextInt(Integer.parseInt(rocksnum));
				jedis.hset("rocksid", set, rocksid + "");
			}
		} finally {
			rpL1.putInstance(jedis);
		}
